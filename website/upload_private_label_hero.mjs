...
async function upload() {
  const imageUrl = "https://sc02.alicdn.com/kf/H586ed5bc53904736b36e8939f7688552O.jpg";
  const response = await fetch(imageUrl);
  const buffer = await response.buffer();

  await s3.send(new PutObjectCommand({
    Bucket: "peakmotion-assets",
    Key: "blog-private-label-socks-hero.jpg",
    Body: buffer,
    ContentType: "image/jpeg",
  }));

  console.log("Upload successful: blog-private-label-socks-hero.jpg");
}
