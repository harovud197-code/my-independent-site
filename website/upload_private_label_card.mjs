...
async function upload() {
  const imageUrl = "https://sc02.alicdn.com/kf/Hb88b314246194a4aae62708ad87cdbb6G.jpg";
  const response = await fetch(imageUrl);
  const buffer = await response.buffer();

  await s3.send(new PutObjectCommand({
    Bucket: "peakmotion-assets",
    Key: "blog-private-label-socks-card.jpg",
    Body: buffer,
    ContentType: "image/jpeg",
  }));

  console.log("Upload successful: blog-private-label-socks-card.jpg");
}
