...
async function upload() {
  const imageUrl = "https://sc02.alicdn.com/kf/Ha88119d4528748aa9c68b37a86b70405b.jpg";
  const response = await fetch(imageUrl);
  const buffer = await response.buffer();

  await s3.send(new PutObjectCommand({
    Bucket: "peakmotion-assets",
    Key: "products-pilates-low-cut-colors.jpg",
    Body: buffer,
    ContentType: "image/jpeg",
  }));

  console.log("Upload successful: products-pilates-low-cut-colors.jpg");
}
