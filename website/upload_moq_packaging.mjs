...
async function upload() {
  const imageUrl = "https://sc02.alicdn.com/kf/H8d9242a62d1b4876b49461070ff4ef6e7.jpg";
  const response = await fetch(imageUrl);
  const buffer = await response.buffer();

  await s3.send(new PutObjectCommand({
    Bucket: "peakmotion-assets",
    Key: "blog-moq-guide-packaging.jpg",
    Body: buffer,
    ContentType: "image/jpeg",
  }));

  console.log("Upload successful: blog-moq-guide-packaging.jpg");
}
