...
async function upload() {
  const imageUrl = "https://sc02.alicdn.com/kf/He9e7c8b8e4974359bda0fc43b791482cm.jpg";
  const response = await fetch(imageUrl);
  const buffer = await response.buffer();

  await s3.send(new PutObjectCommand({
    Bucket: "peakmotion-assets",
    Key: "blog-private-label-vs-oem-infographic.jpg",
    Body: buffer,
    ContentType: "image/jpeg",
  }));

  console.log("Upload successful: blog-private-label-vs-oem-infographic.jpg");
}
