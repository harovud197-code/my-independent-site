  ...
  return (
    <main className="bg-white text-[#0c1713]">
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-20">
          ...
          <div className="relative min-h-[380px] overflow-hidden rounded-lg bg-[#0c1713] p-7 shadow-[0_28px_65px_rgba(12,23,19,0.22)] md:min-h-[520px] md:p-12">
            ...
            <div className="relative flex h-full flex-col justify-between">
              ...
              <Image src="/assets/images/training/hero.png" alt="Custom grip socks in multiple colors" width={700} height={560} priority className="mx-auto h-[270px] w-full object-contain md:h-[360px]" />
      ...
      <section className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-10 px-5 py-16 md:px-8 lg:grid-cols-[0.9fr_1.1fr] lg:py-24">
        <div className="relative aspect-[4/3] overflow-hidden rounded-lg bg-slate-100"><Image src={`${assetPath}/factory/sock-knitting-workshop.jpg`} alt="Computerized sock knitting machines in the PeakMotion factory" fill sizes="(max-width: 1024px) 100vw, 45vw" className="object-cover" /></div>
        <div><p className="text-sm font-black uppercase tracking-[0.18em] text-[#679a20]">Factory and Quality Control</p><h2 className="mt-3 text-3xl font-black md:text-5xl">Production support buyers can see</h2><p className="mt-5 text-lg leading-relaxed text-slate-600">From yarn preparation and computerized knitting to boarding, finishing and packing, the key production stages are handled by an experienced factory team in Haiyan, Zhejiang.</p><div className="mt-7 grid grid-cols-2 gap-3 text-sm font-bold text-slate-700">{['Yarn inspection', 'Knitting checks', 'Silicone grip inspection', 'Size measurement', 'Logo inspection', 'Packaging verification'].map(item => <span key={item} className="rounded-lg border border-slate-200 bg-[#f7f8f6] p-4">{item}</span>)}</div><a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="mt-8 inline-flex rounded-xl bg-[#0c1713] px-6 py-4 text-sm font-black uppercase tracking-wide text-white transition-colors hover:bg-[#163020]">Discuss Your Specification</a></div>
      </section>

      <section className="bg-[#f7f8f6] px-5 py-16 md:px-8 lg:py-24">
        <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-10 lg:grid-cols-[1.1fr_0.9fr]"><div><p className="text-sm font-black uppercase tracking-[0.18em] text-[#679a20]">Product and Factory Video</p><h2 className="mt-3 text-3xl font-black md:text-5xl">See our customization and production capability</h2><p className="mt-5 text-lg leading-relaxed text-slate-600">See our factory overview, then send your design requirements for a practical product recommendation and quotation direction.</p><div className="mt-7 space-y-3 text-sm font-bold text-slate-700"><p className="rounded-lg border border-slate-200 bg-white p-4">Custom grip patterns, materials, colors and logo methods</p><p className="rounded-lg border border-slate-200 bg-white p-4">OEM and ODM support from sampling through shipment</p><p className="rounded-lg border border-slate-200 bg-white p-4">Retail-ready packaging for brands, studios and wholesalers</p></div></div><div className="overflow-hidden rounded-lg bg-[#0c1713] shadow-[0_24px_55px_rgba(12,23,19,0.2)]"><Image src={`${assetPath}/videos/custom-grip-socks-production-poster.jpg`} alt="PeakMotion custom grip socks production facility" width={1200} height={675} className="aspect-video w-full object-cover" /><div className="border-t border-white/10 px-6 py-5 text-sm font-bold text-white">Production videos and live factory walkthroughs are available during your quotation consultation.</div></div></div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 md:px-8 lg:py-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-6 lg:grid-cols-3">{[
          ['United States buyer', 'Second order from this company and we are very pleased. The socks were produced quickly with clear communication and attention to detail.', 'review-united-states-may-2026.png'],
          ['United Kingdom buyer', 'The products arrived as described and on schedule. Communication was great from start to finish.', 'review-united-kingdom-may-2026.png'],
          ['Czech Republic buyer', 'The socks, print and packaging turned out exactly as expected. Detailed photos and videos before shipping built confidence.', 'review-czech-republic-jul-2026.png'],
        ].map(([buyer, quote, image]) => <article key={buyer} className="overflow-hidden rounded-lg border border-slate-200 bg-white"><div className="p-7"><p className="text-xs font-black uppercase tracking-[0.16em] text-[#679a20]">5-star buyer feedback</p><h3 className="mt-4 text-lg font-black">{buyer}</h3><p className="mt-4 leading-relaxed text-slate-600">&ldquo;{quote}&rdquo;</p></div><div className="border-t border-slate-100 bg-[#f7f8f6] p-4"><Image src={`${assetPath}/reviews/${image}`} alt={`Marketplace review from a ${buyer}`} width={900} height={520} className="h-auto w-full rounded border border-slate-200" /></div></article>)}</div>
      </section>
