...
const faqs = [
  ...
  ['What is the MOQ for custom grip socks?', 'Custom logo and studio grip programs typically start from 100 pairs. Selected standard styles can start from 50 pairs, while bespoke construction or packaging requires a higher MOQ depending on the specification.'],
        ...
        <div className="mx-auto max-w-4xl px-5 py-12 md:px-8 md:py-16">
          ...
          <section className="mt-16 scroll-mt-24" id="cost-factors">
            ...
            <div className="mt-8 grid gap-5 md:grid-cols-2">
              {[['Order quantity and MOQ', 'Selected standard styles can start from 50 pairs. Custom logo and studio grip programs typically start from 100 pairs; bespoke specifications require a higher MOQ.'], ['Materials and construction', 'Cotton blends can suit comfort-led programs, while nylon, polyester and performance blends can be selected around durability, stretch and moisture management.'], ['Silicone grip design', 'Basic dots, targeted strips, custom logo grips and fuller sole coverage each involve different materials, artwork and application requirements.'], ['Branding and packaging', 'Jacquard details, labels, colorways, individual bags, header cards and retail packaging can add production steps and affect the final quote.']].map(([title, text]) => <article key={title} className="rounded-2xl border border-slate-200 bg-[#f7f8f6] p-7"><h3 className="text-xl font-black">{title}</h3><p className="mt-3 leading-relaxed text-slate-600">{text}</p></article>)}
            </div>
