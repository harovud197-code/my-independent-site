    ...
    <main className="bg-white text-[#0c1713]">
      ...
      <section className="px-5 py-16 md:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-3">{caseStudies.map((item) => <article key={item.href} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_16px_42px_rgba(12,23,19,0.07)]"><div className="relative aspect-video"><Image src={item.image} alt={item.alt} fill sizes="(max-width: 1024px) 100vw, 33vw" className="object-cover" /></div><div className="p-7"><p className="text-xs font-black uppercase tracking-[0.16em] text-[#679a20]">{item.industry}</p><h3 className="mt-3 text-2xl font-black">{item.title}</h3><Link href={item.href} className="mt-6 inline-flex font-black text-[#315d18] underline decoration-[#b4ff2b] decoration-2 underline-offset-4">Read Case Study →</Link></div></article>)}</div>
        </div>
      </section>
