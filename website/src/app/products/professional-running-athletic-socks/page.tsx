...
export const metadata: Metadata = {
  title: 'Professional Running Athletic Socks | PeakMotion Socks',
  description: 'Professional compression sports socks with breathable mesh, support zones and OEM private label customization.',
};
  ...
  return (
    <main className="bg-white text-[#0c1713]">
          ...
          <div className="grid grid-cols-2 overflow-hidden rounded-2xl border border-slate-200 sm:grid-cols-4">
            {[['MOQ', '50 Pairs'], ['Sample', '7 Days'], ['Lead Time', '25-30 Days'], ['Size', 'Kids-XL']].map(([label, value]) => (
              <div key={label} className="border-b border-r border-slate-200 p-4 text-center last:border-r-0 sm:border-b-0">
                <span className="block text-xs font-bold uppercase tracking-wider text-slate-500">{label}</span><strong className="mt-1 block text-sm">{value}</strong>
              </div>
            ))}
          </div>
      ...
      <section className="mx-auto grid max-w-[1200px] grid-cols-1 gap-12 px-5 py-20 md:px-8 lg:grid-cols-2">
        ...
        <div><p className="mb-3 text-sm font-black uppercase tracking-[0.15em] text-[#679a20]">For Your Brand</p><h2 className="text-3xl font-black md:text-4xl">OEM &amp; Customization</h2><img src={`${assetPath}/oem.png`} alt="OEM customization options" className="mt-8 h-auto w-full rounded-2xl" /></div>
      </section>
      ...
      <section className="bg-[#f7f8f6] py-20"><div className="mx-auto max-w-[1200px] space-y-14 px-5 md:px-8"><div className="text-center"><p className="text-sm font-black uppercase tracking-[0.15em] text-[#679a20]">Custom Options</p><h2 className="mt-3 text-3xl font-black md:text-4xl">Colors, applications and factory support</h2></div>
        {['colors.png', 'applications.png', 'factory.png', 'certifications.png'].map((image) => <img key={image} src={`${assetPath}/${image}`} alt="Running socks product information" className="h-auto w-full rounded-2xl bg-white" />)}
      </div></section>
