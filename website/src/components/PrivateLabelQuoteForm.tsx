...
export default function PrivateLabelQuoteForm({ id = 'quote' }: PrivateLabelQuoteFormProps) {
  function submitQuote(event: FormEvent<HTMLFormElement>) {
    ...
    const lines = [
      'Hello PeakMotion Socks, I would like to develop private label sports socks.',
      `Name: ${formData.get('name') || 'Not provided'}`,
      `Business email: ${formData.get('email') || 'Not provided'}`,
      `Product type: ${formData.get('productType') || 'Not selected'}`,
      `Brand stage: ${formData.get('brandStage') || 'Not selected'}`,
  ...
  return (
    <form id={id} onSubmit={submitQuote} className="relative overflow-hidden rounded-2xl bg-[#0c1713] p-7 text-white shadow-[0_28px_65px_rgba(12,23,19,0.22)] md:p-9">
      ...
      <div className="relative">
        <p className="text-xs font-black uppercase tracking-[0.16em] text-[#b4ff2b]">Start a private-label project</p>
        <h2 className="mt-3 text-3xl font-black tracking-tight">Tell us what you want to launch</h2>
        <p className="mt-3 leading-relaxed text-slate-300">Share your product idea, brand stage and order needs. We will reply with a practical next step for sampling and production.</p>
