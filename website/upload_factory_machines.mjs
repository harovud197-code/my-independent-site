...
async function upload() {
  const imageUrl = "https://sc02.alicdn.com/kf/H9d71276a03414227922c7f16a81e89c4a.jpg";
  const response = await fetch(imageUrl);
  const buffer = await response.buffer();

  await s3.send(new PutObjectCommand({
    Bucket: "peakmotion-assets",
    Key: "blog-private-label-factory-machines.jpg",
    Body: buffer,
    ContentType: "image/jpeg",
  }));

  console.log("Upload successful: blog-private-label-factory-machines.jpg");
}
