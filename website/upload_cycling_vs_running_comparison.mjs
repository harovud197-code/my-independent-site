...
async function upload() {
  const imageUrl = "https://sc04.alicdn.com/kf/H5b610a167bb4412c9bc9c7484df1052dj.jpg";
  const response = await fetch(imageUrl);
  const buffer = await response.buffer();

  await s3.send(new PutObjectCommand({
    Bucket: "peakmotion-assets",
    Key: "blog-cycling-vs-running-comparison.jpg",
    Body: buffer,
    ContentType: "image/jpeg",
  }));

  console.log("Upload successful: blog-cycling-vs-running-comparison.jpg");
}
